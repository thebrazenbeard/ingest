...
Raw and normalized artifacts are immutable. Artifact identity is SHA-256 of exact persisted bytes. Ingest identity is a canonical digest over stable source identity, raw SHA-256, parser-driving media type, normalizer version, and policy identity. Store reads are fail-closed: records recursively verify canonical JSON, receipt self-digests, derivation identity, content-addressed blob bytes, and receipt/record cross-evidence before returning. `ingest inspect` reports corrupted records as machine-readable `CORRUPT` with exit code `2` instead of returning unverified evidence.
